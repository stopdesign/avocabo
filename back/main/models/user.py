...
class User(AbstractUser):
    ...
    def update_rotation(self):
        ...
        """
        from main.models import Word, Attempt, UserWord
        ...
        # все слова которые выключены у юзера
        hidden_word_ids = UserWord.objects.filter(user_id=self.pk).values('word_id')

        # фильтр по активным группам пользователя
        words = Word.objects.filter(list__in=list_ids, id__in=rotation)
        words = words.exclude(definitions=None)
        words = words.exclude(id__in=hidden_word_ids)
        words = words.values_list('id', flat=True)

        # все слова, которые потенциально можно добавить
        words_count = Word.objects.filter(list__in=list_ids).exclude(definitions=None).exclude(id__in=hidden_word_ids).count()
        ...
        if to_add > 0:
            ...
            words = Word.objects.filter(list__in=list_ids)
            words = words.exclude(definitions=None)
            words = words.exclude(id__in=new_r_words)
            words = words.exclude(id__in=hidden_word_ids)
